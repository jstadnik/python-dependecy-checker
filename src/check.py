...
def get_python_bits_from_soup(soup):
    ...
    python_bits = [
        text_string for text_string in text_strings if "Python" in text_string
    ]
    return python_bits
...
def print_results(unknown, incompatible):
    ...
    if unknown:
        ...
        for key, value in unknown.items():
            print(f"{key}   Newest supported: {value}")
    ...
    if incompatible:
        ...
        for key, value in incompatible.items():
            print(
                f"{key}  Supported versions:{value['supported_versions']}   Newest version supported: {value['newest_supported']}"
            )
...
def check(python_version, filepath):
    ...
    for dependency, version in dependencies:
        ...
        status, supported_versions = check_dependency(
            python_version, dependency, version
        )
        if status is not Status.SUPPORTED:
            ...
            if status is Status.UNKNOWN:
                ...
                incompatible[dependency] = {
                    ...
                    "newest_supported": status_newest.desc(),
                }
        # Don't ddos pypi
        time.sleep(0.5)
